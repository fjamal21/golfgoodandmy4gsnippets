...
class EditView extends AbstractContentForm
{
    ...
    {
        $min = htmlspecialchars( $this->content->getMeta('minimum_raise') ) / 100;
        $val = htmlspecialchars( $this->content->getMeta('retail_value') ) / 100;
	...
	if ( $this->content->getMeta('prize_name') ) echo ' value="' . htmlspecialchars( $this->content->getMeta('prize_name') ) . '"';
	echo ' />
		</div>
	    ';
	...
	if ( $this->content->getMeta('display_order') ) echo ' value="' . htmlspecialchars( $this->content->getMeta('display_order') ) . '"';
	echo ' />
		</div>
	    ';
	...
	if ( $this->content->getMeta('quantity') ) echo ' value="' . htmlspecialchars( $this->content->getMeta('quantity') ) . '"';
	echo ' />
		</div>
	    ';
	...
	if ( $this->content->getMeta('minimum_raise') ) echo ' value="' . $min . '"';
	echo ' />
		</div>
	    ';
	...
	if ( $this->content->getMeta('retail_value') ) echo ' value="' . $val . '"';
	echo ' />
		</div>
	    ';
    }
